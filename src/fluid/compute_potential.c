...
static int init_eigenvalues(
  ...
  const sdecomp_info_t * info = domain->info;
  double * restrict * eval_ys = &poisson_solver->eval_ys;
#if NDIMS == 3
  double * restrict * eval_zs = &poisson_solver->eval_zs;
#endif
#if NDIMS == 2
  // x1 pencil, DFT in y direction
  ...
  // azimuthal eigenvalues | 9
  *eval_ys = memory_calloc(mysizes[0], sizeof(double));
  for(size_t local_n = 0; local_n < mysizes[0]; local_n++){
    const size_t global_n = local_n + offsets[0];
    (*eval_ys)[local_n] =
      - 4. * pow(
        sin( g_pi * global_n / signal_lengths[0] ),
        2.
      );
  }
#if NDIMS == 3
  // axial eigenvalues | 9
  *eval_zs = memory_calloc(mysizes[1], sizeof(double));
  for(size_t local_n = 0; local_n < mysizes[1]; local_n++){
    const size_t global_n = local_n + offsets[1];
    (*eval_zs)[local_n] =
      - 4. * pow(
        sin( g_pi * global_n / signal_lengths[1] ),
        2.
      );
  }
#endif
